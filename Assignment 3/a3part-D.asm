;
; a3part-D.asm
;
; Part D of assignment #3
;
;
; Student name: Lucas Hewgill
; Student ID: V01033481
; Date of completed work:
;
; **********************************
; Code provided for Assignment #3
;
; Author: Mike Zastre (2024-Nov-04)
;
; This skeleton of an assembly-language program is provided to help you 
; begin with the programming tasks for A#3. As with A#2 and A#1, there are
; "DO NOT TOUCH" sections. You are *not* to modify the lines within these
; sections. The only exceptions are for specific changes announced on
; Brightspace or in written permission from the course instruction.
; *** Unapproved changes could result in incorrect code execution
; during assignment evaluation, along with an assignment grade of zero. ***
;


; =============================================
; ==== BEGINNING OF "DO NOT TOUCH" SECTION ====
; =============================================
;
; In this "DO NOT TOUCH" section are:
; 
; (1) assembler direction setting up the interrupt-vector table
;
; (2) "includes" for the LCD display
;
; (3) some definitions of constants that may be used later in
;     the program
;
; (4) code for initial setup of the Analog-to-Digital Converter
;     (in the same manner in which it was set up for Lab #4)
;
; (5) Code for setting up three timers (timers 1, 3, and 4).
;
; After all this initial code, your own solutions's code may start
;

.cseg
.org 0
    jmp reset

; Actual .org details for this an other interrupt vectors can be
; obtained from main ATmega2560 data sheet
;
.org 0x22
    jmp timer1

; This included for completeness. Because timer3 is used to
; drive updates of the LCD display, and because LCD routines
; *cannot* be called from within an interrupt handler, we
; will need to use a polling loop for timer3.
;
; .org 0x40
;    jmp timer3

.org 0x54
    jmp timer4

.include "m2560def.inc"
.include "lcd.asm"

.cseg
#define CLOCK 16.0e6
#define DELAY1 0.01
#define DELAY3 0.1
#define DELAY4 0.5

#define BUTTON_RIGHT_MASK 0b00000001    
#define BUTTON_UP_MASK    0b00000010
#define BUTTON_DOWN_MASK  0b00000100
#define BUTTON_LEFT_MASK  0b00001000

#define BUTTON_RIGHT_ADC  0x032
#define BUTTON_UP_ADC     0x0b0   ; was 0x0c3
#define BUTTON_DOWN_ADC   0x160   ; was 0x17c
#define BUTTON_LEFT_ADC   0x22b
#define BUTTON_SELECT_ADC 0x316

.equ PRESCALE_DIV=1024   ; w.r.t. clock, CS[2:0] = 0b101

; TIMER1 is a 16-bit timer. If the Output Compare value is
; larger than what can be stored in 16 bits, then either
; the PRESCALE needs to be larger, or the DELAY has to be
; shorter, or both.
.equ TOP1=int(0.5+(CLOCK/PRESCALE_DIV*DELAY1))
.if TOP1>65535
.error "TOP1 is out of range"
.endif

; TIMER3 is a 16-bit timer. If the Output Compare value is
; larger than what can be stored in 16 bits, then either
; the PRESCALE needs to be larger, or the DELAY has to be
; shorter, or both.
.equ TOP3=int(0.5+(CLOCK/PRESCALE_DIV*DELAY3))
.if TOP3>65535
.error "TOP3 is out of range"
.endif

; TIMER4 is a 16-bit timer. If the Output Compare value is
; larger than what can be stored in 16 bits, then either
; the PRESCALE needs to be larger, or the DELAY has to be
; shorter, or both.
.equ TOP4=int(0.5+(CLOCK/PRESCALE_DIV*DELAY4))
.if TOP4>65535
.error "TOP4 is out of range"
.endif

reset:
; ***************************************************
; **** BEGINNING OF FIRST "STUDENT CODE" SECTION ****
; ***************************************************

; Anything that needs initialization before interrupts
; start must be placed here.

.def DATAH=r25          ; High byte of ADC data
.def DATAL=r24          ; Low byte of ADC data
.def BOUNDARY_H = r1    ; High byte of boundary value
.def BOUNDARY_L = r0    ; Low byte of boundary value
.def BOUNDARY_BH = r3   ; High byte of button boundary
.def BOUNDARY_BL = r2   ; Low byte of button boundary
                        ; From lab 7
.equ ADCSRA_BTN=0x7A    ; ADC Control and Status Register A
.equ ADCSRB_BTN=0x7B    ; ADC Control and Status Register B
.equ ADMUX_BTN=0x7C     ; ADC Multiplexer Selection Register
.equ ADCL_BTN=0x78      ; ADC Data Register Low Byte
.equ ADCH_BTN=0x79      ; ADC Data Register High Byte

; Part C
; Reset the character settings to 0
clear_index:
    ldi r18, 0
    ldi XL, LOW(CURRENT_CHARSET_INDEX)    ; Load low byte of CURRENT_CHARSET_INDEX address
    ldi XH, HIGH(CURRENT_CHARSET_INDEX)   ; Load high byte of CURRENT_CHARSET_INDEX address
    sts CURRENT_CHARSET_INDEX, r18        ; Reset CURRENT_CHARSET_INDEX to 0
    sts CURRENT_CHAR_INDEX, r18           ; Reset CURRENT_CHAR_INDEX to 0

    ; Part D
    ldi r19, ' '                          ; Load space character into r19
    clr r24                               ; Clear r24 (used as a pointer or counter)
    ldi YL, LOW(TOP_LINE_CONTENT)         ; Load low byte of TOP_LINE_CONTENT address
    ldi YH, HIGH(TOP_LINE_CONTENT)        ; Load high byte of TOP_LINE_CONTENT address
    sts TOP_LINE_CONTENT, r19             ; Initialize TOP_LINE_CONTENT with spaces
    ldi r25, 16                           ; Loop counter for 16 bytes

clear_loop:      ; Loop 16 times to initialize arrays
    st X+, r18                            ; Store 0 into CURRENT_CHARSET_INDEX array and increment pointer
    st Y+, r19                            ; Store space into TOP_LINE_CONTENT array and increment pointer
    dec r25                               ; Decrement loop counter
    brne clear_loop                       ; Continue loop if r25 is not zero
    
    ; This gets the start of the available character set
access_letter:
    ldi ZL, LOW(AVAILABLE_CHARSET << 1)   ; Load low byte of AVAILABLE_CHARSET address into ZL
    ldi ZH, HIGH(AVAILABLE_CHARSET << 1)  ; Load high byte of AVAILABLE_CHARSET address into ZH

get_length: ; Calculate the length of AVAILABLE_CHARSET
    lpm r16, Z+                           ; Load program memory into r16 and increment Z pointer
    cpi r16, 0x00                         ; Compare loaded byte with null terminator
    breq store_length                     ; If null terminator found, store the length
    inc r24                               ; Increment length counter
    rjmp get_length                       ; Repeat until null terminator is found

store_length:    ; Store the length of the string into LENGTH
    sts LENGTH, r24

; ***************************************************
; ******* END OF FIRST "STUDENT CODE" SECTION *******
; ***************************************************

; =============================================
; ====  START OF "DO NOT TOUCH" SECTION    ====
; =============================================

    ; Initialize the ADC converter (which is needed
    ; to read buttons on shield). Note that we'll
    ; use the interrupt handler for timer 1 to
    ; read the buttons (i.e., every 10 ms)
    ;
    ldi temp, (1 << ADEN) | (1 << ADPS2) | (1 << ADPS1) | (1 << ADPS0)
    sts ADCSRA, temp
    ldi temp, (1 << REFS0)
    sts ADMUX, r16

    ; Timer 1 is for sampling the buttons at 10 ms intervals.
    ; We will use an interrupt handler for this timer.
    ldi r17, high(TOP1)
    ldi r16, low(TOP1)
    sts OCR1AH, r17
    sts OCR1AL, r16
    clr r16
    sts TCCR1A, r16
    ldi r16, (1 << WGM12) | (1 << CS12) | (1 << CS10)
    sts TCCR1B, r16
    ldi r16, (1 << OCIE1A)
    sts TIMSK1, r16

    ; Timer 3 is for updating the LCD display. We are
    ; *not* able to call LCD routines from within an 
    ; interrupt handler, so this timer must be used
    ; in a polling loop.
    ldi r17, high(TOP3)
    ldi r16, low(TOP3)
    sts OCR3AH, r17
    sts OCR3AL, r16
    clr r16
    sts TCCR3A, r16
    ldi r16, (1 << WGM32) | (1 << CS32) | (1 << CS30)
    sts TCCR3B, r16
    ; Notice that the code for enabling the Timer 3
    ; interrupt is missing at this point.

    ; Timer 4 is for updating the contents to be displayed
    ; on the top line of the LCD.
    ldi r17, high(TOP4)
    ldi r16, low(TOP4)
    sts OCR4AH, r17
    sts OCR4AL, r16
    clr r16
    sts TCCR4A, r16
    ldi r16, (1 << WGM42) | (1 << CS42) | (1 << CS40)
    sts TCCR4B, r16
    ldi r16, (1 << OCIE4A)
    sts TIMSK4, r16

    sei

; =============================================
; ====    END OF "DO NOT TOUCH" SECTION    ====
; =============================================

; ****************************************************
; **** BEGINNING OF SECOND "STUDENT CODE" SECTION ****
; ****************************************************

rcall lcd_init  ; Initialize the LCD display

start: ; Polling loop for Timer 3
    in r16, TIFR3         ; Read Timer 3 Interrupt Flag Register
    sbrs r16, OCF3A       ; Skip next instruction if Output Compare Flag is set
    rjmp start            ; Continue polling if flag is not set

    ldi r16, (1 << OCF3A) ; Prepare to clear the Output Compare Flag
    out TIFR3, r16        ; Clear the Output Compare Flag
    rjmp timer3           ; Jump to the Timer 3 handler

stop:
    rjmp stop             ; Infinite loop to halt execution

; Timer1 Interrupt Service Routine
timer1: 
    ; Part A: Read button state using ADC every 10 ms

    ; Save registers that will be used
    push r16
    lds r16, SREG
    push r16
    push DATAL
    push DATAH
    push BOUNDARY_L
    push BOUNDARY_H
    push r23
    push BOUNDARY_BL
    push BOUNDARY_BH
    push r17

    ; Start ADC conversion
    lds r16, ADCSRA_BTN  
    ori r16, 0x40         ; Set ADSC bit to start conversion
    sts ADCSRA_BTN, r16

wait: ; Wait for ADC conversion to complete
    lds r16, ADCSRA_BTN
    andi r16, 0x40        ; Check ADSC bit
    brne wait             ; Wait until ADSC is cleared

    ; Read ADC result into DATAH:DATAL
    lds DATAL, ADCL_BTN
    lds DATAH, ADCH_BTN

    ; Load boundary for button select detection
    ldi r16, low(BUTTON_SELECT_ADC)
    mov BOUNDARY_L, r16
    ldi r16, high(BUTTON_SELECT_ADC)
    mov BOUNDARY_H, r16

    ; Determine if button is pressed based on ADC value
    clr r23               ; Clear temporary register r23
    cp DATAL, BOUNDARY_L  ; Compare ADC low byte with boundary low byte
    cpc DATAH, BOUNDARY_H ; Compare ADC high byte with boundary high byte
    brsh btn_pressed      ; If ADC value >= boundary, branch to btn_pressed
    ldi r23, 1            ; Set r23 to 1 (button not pressed)

btn_pressed:  
    sts BUTTON_IS_PRESSED, r23  ; Update button pressed flag

    ; Part B: Identify which button is pressed
checkR:
    ldi r16, low(BUTTON_RIGHT_ADC)
    mov BOUNDARY_BL, r16
    ldi r16, high(BUTTON_RIGHT_ADC)
    mov BOUNDARY_BH, r16
    cp DATAL, BOUNDARY_BL
    cpc DATAH, BOUNDARY_BH
    brlo lcd_right         ; If ADC value < BUTTON_RIGHT_ADC, branch to lcd_right

checkU:
    ldi r16, low(BUTTON_UP_ADC)
    mov BOUNDARY_BL, r16
    ldi r16, high(BUTTON_UP_ADC)
    mov BOUNDARY_BH, r16
    cp DATAL, BOUNDARY_BL
    cpc DATAH, BOUNDARY_BH
    brlo lcd_up            ; If ADC value < BUTTON_UP_ADC, branch to lcd_up

checkD:
    ldi r16, low(BUTTON_DOWN_ADC)
    mov BOUNDARY_BL, r16
    ldi r16, high(BUTTON_DOWN_ADC)
    mov BOUNDARY_BH, r16
    cp DATAL, BOUNDARY_BL
    cpc DATAH, BOUNDARY_BH
    brlo lcd_down          ; If ADC value < BUTTON_DOWN_ADC, branch to lcd_down

checkL:
    ldi r16, low(BUTTON_LEFT_ADC)
    mov BOUNDARY_BL, r16
    ldi r16, high(BUTTON_LEFT_ADC)
    mov BOUNDARY_BH, r16
    cp DATAL, BOUNDARY_BL
    cpc DATAH, BOUNDARY_BH
    brlo lcd_left          ; If ADC value < BUTTON_LEFT_ADC, branch to lcd_left

lcd_right:
    ldi r17, 'R'                   ; Load character 'R' for Right button
    sts LAST_BUTTON_PRESSED, r17   ; Update last button pressed
    rjmp endT1

lcd_up:
    ldi r17, 'U'                   ; Load character 'U' for Up button
    sts LAST_BUTTON_PRESSED, r17
    rjmp endT1

lcd_down:
    ldi r17, 'D'                   ; Load character 'D' for Down button
    sts LAST_BUTTON_PRESSED, r17
    rjmp endT1

lcd_left:
    ldi r17, 'L'                   ; Load character 'L' for Left button
    sts LAST_BUTTON_PRESSED, r17
    rjmp endT1

endT1:
    ; Restore saved registers
    pop r17
    pop BOUNDARY_BH
    pop BOUNDARY_BL
    pop r23
    pop BOUNDARY_H
    pop BOUNDARY_L
    pop DATAH
    pop DATAL
    pop r16
    sts SREG, r16
    pop r16

    reti  ; Return from interrupt

; Timer3 Handler (called from polling loop)
timer3:
    ; Part A: Display '*' or '-' based on BUTTON_IS_PRESSED flag
    push r16
    push r17
    push r18
    push r19
    push r20
    push r23
    push YH
    push YL

    ; Set LCD cursor position to column 15, row 1
    ldi r16, 1                     ; Row 1
    ldi r17, 15                    ; Column 15
    push r16
    push r17
    rcall lcd_gotoxy               ; Move cursor to specified position
    pop r17
    pop r16

    ; Check if the button is pressed
    clr r18
    lds r18, BUTTON_IS_PRESSED
    cpi r18, 0x01
    breq star_on                   ; If pressed, display '*'

dash_on: ; Display '-'
    ldi r18, '-'
    push r18
    rcall lcd_putchar              ; Display '-'
    pop r18
    rjmp endT3

star_on: ; Display '*'
    ldi r18, '*'
    push r18
    rcall lcd_putchar              ; Display '*'
    pop r18

    ; Part B: Display direction of last button pressed
direction_on:
    ; Set LCD cursor to row 0, column 1
    ldi r16, 1                     ; Row 1
    ldi r17, 0                     ; Column 0
    push r16
    push r17
    rcall lcd_gotoxy               ; Move cursor to specified position
    pop r17
    pop r16

    ; Load LAST_BUTTON_PRESSED
    clr r19
    lds r19, LAST_BUTTON_PRESSED
    ldi r20, ' '                   ; Space character

    cpi r19, 'L'
    breq left_on

    cpi r19, 'D'
    breq down_on

    cpi r19, 'U'
    breq up_on

    cpi r19, 'R'
    breq right_on

left_on:
    push r19                       ; Display 'L' for Left
    rcall lcd_putchar
    pop r19
    push r20                       ; Display spaces for other directions
    rcall lcd_putchar
    pop r20
    push r20
    rcall lcd_putchar
    pop r20
    push r20
    rcall lcd_putchar
    pop r20
    rjmp hexdigit_on               ; Proceed to Part D

down_on:
    push r20                       ; Display space before 'D'
    rcall lcd_putchar
    pop r20
    push r19                       ; Display 'D' for Down
    rcall lcd_putchar
    pop r19
    push r20                       ; Display spaces for other directions
    rcall lcd_putchar
    pop r20
    push r20
    rcall lcd_putchar
    pop r20
    rjmp hexdigit_on               ; Proceed to Part D

up_on:
    push r20                       ; Display spaces before 'U'
    rcall lcd_putchar
    pop r20
    push r20
    rcall lcd_putchar
    pop r20
    push r19                       ; Display 'U' for Up
    rcall lcd_putchar
    pop r19
    push r20                       ; Display space after 'U'
    rcall lcd_putchar
    pop r20
    rjmp hexdigit_on               ; Proceed to Part D

right_on:
    push r20                       ; Display spaces before 'R'
    rcall lcd_putchar
    pop r20
    push r20
    rcall lcd_putchar
    pop r20
    push r20
    rcall lcd_putchar
    pop r20
    push r19                       ; Display 'R' for Right
    rcall lcd_putchar
    pop r19
    rjmp hexdigit_on               ; Proceed to Part D

; Part C: Display current character from TOP_LINE_CONTENT
hexdigit_on:

    ; Set cursor to row 0, column CURRENT_CHAR_INDEX
    ldi r16, 0                      ; Row 0
    lds r17, CURRENT_CHAR_INDEX     ; Load CURRENT_CHAR_INDEX into r17
    push r16
    push r17
    rcall lcd_gotoxy                ; Move cursor to specified position
    pop r17
    pop r16

    ; Get the current character from TOP_LINE_CONTENT and display it
    clr r23
    ldi YL, LOW(TOP_LINE_CONTENT)   ; Load low byte of TOP_LINE_CONTENT address
    ldi YH, HIGH(TOP_LINE_CONTENT)  ; Load high byte of TOP_LINE_CONTENT address
    ; Y points to TOP_LINE_CONTENT + CURRENT_CHAR_INDEX
    ld r23, Y                       ; Load character into r23

    push r23
    rcall lcd_putchar               ; Display the character
    pop r23

endT3:
    ; Restore saved registers
    pop YL
    pop YH
    pop r23
    pop r20
    pop r19
    pop r18
    pop r17
    pop r16
    rjmp start                      ; Return to polling loop

; timer3:
;
; Note: There is no "timer3" interrupt handler as you must use
; timer3 in a polling style (i.e. it is used to drive the refreshing
; of the LCD display, but LCD functions cannot be called/used from
; within an interrupt handler).


; Timer4 Interrupt Service Routine
timer4:
    ; Part C: Update TOP_LINE_CONTENT based on button presses
    push r27
    push r22
    push ZL
    push ZH
    lds r27, SREG
    push r27
    push r23
    push ZL
    push ZH
    push r29
    push r28
    push r30

    lds r22, BUTTON_IS_PRESSED      ; Check if the button is pressed
    cpi r22, 0
    breq endT4                      ; If not pressed, exit interrupt

    lds r22, LAST_BUTTON_PRESSED    ; Load last button pressed
    
    cpi r22, 'U'
    breq go_up

    cpi r22, 'D'
    breq go_down

    cpi r22, 'R'    
    breq move_right

    cpi r22, 'L'
    breq move_left

go_down:
    lds r27, CURRENT_CHARSET_INDEX
    dec r27                         ; Decrement character set index
    rjmp check_boundary

go_up:
    lds r27, CURRENT_CHARSET_INDEX
    inc r27                         ; Increment character set index
    rjmp check_boundary

move_right:                          ; Part D: Move cursor right
    lds r27, CURRENT_CHAR_INDEX
    inc r27                         ; Increment character index
    sts CURRENT_CHAR_INDEX, r27     ; Update CURRENT_CHAR_INDEX
    rjmp index_to_charset

move_left:                           ; Part D: Move cursor left
    lds r27, CURRENT_CHAR_INDEX
    dec r27                         ; Decrement character index
    sts CURRENT_CHAR_INDEX, r27     ; Update CURRENT_CHAR_INDEX
    rjmp index_to_charset

check_boundary:
    ; Ensure CURRENT_CHARSET_INDEX stays within boundary
    lds r23, LENGTH                 ; Load length of AVAILABLE_CHARSET
    cp r27, r23
    brsh endT4                      ; If index >= length, exit interrupt
    sts CURRENT_CHARSET_INDEX, r27  ; Update CURRENT_CHARSET_INDEX

index_to_charset:
    ldi ZL, LOW(AVAILABLE_CHARSET << 1) ; Load low byte of AVAILABLE_CHARSET address into ZL
    ldi ZH, HIGH(AVAILABLE_CHARSET << 1); Load high byte into ZH

    lds r28, CURRENT_CHARSET_INDEX  ; Load current character set index into r28

    clr r29                         ; Clear r29 (used for high byte if needed)

    add ZL, r28                     ; Add index to Z pointer
    adc ZH, r29                     ; Add carry to ZH if needed

    lpm r30, Z                      ; Load character from program memory into r30

    sts TOP_LINE_CONTENT, r30       ; Store character into TOP_LINE_CONTENT
    
endT4:
    ; Restore saved registers
    pop r30
    pop r28
    pop r29
    pop ZH
    pop ZL
    pop r23
    pop r27
    sts SREG, r27
    pop ZH
    pop ZL
    pop r22
    pop r27

    reti  ; Return from interrupt

; ****************************************************
; ******* END OF SECOND "STUDENT CODE" SECTION *******
; ****************************************************


; =============================================
; ==== BEGINNING OF "DO NOT TOUCH" SECTION ====
; =============================================

; r17:r16 -- word 1
; r19:r18 -- word 2
; word 1 < word 2? return -1 in r25
; word 1 > word 2? return 1 in r25
; word 1 == word 2? return 0 in r25
;
compare_words:
    ; Compare high bytes first
    cp r17, r19
    breq compare_words_lower_byte

    ; Since high bytes are different, determine result
    ; If C is set from previous cp, it means r17 < r19
    ldi r25, 1
    brcs compare_words_is_less_than
    rjmp compare_words_exit

compare_words_is_less_than:
    ldi r25, -1
    rjmp compare_words_exit

compare_words_lower_byte:
    clr r25
    cp r16, r18
    breq compare_words_exit

    ldi r25, 1
    brcs compare_words_is_less_than  ; Re-use existing logic

compare_words_exit:
    ret

.cseg
AVAILABLE_CHARSET: .db "0123456789abcdef_", 0  ; Null-terminated character set


.dseg

BUTTON_IS_PRESSED: .byte 1         ; Updated by timer1 interrupt, used by LCD update loop
LAST_BUTTON_PRESSED: .byte 1       ; Updated by timer1 interrupt, used by LCD update loop

TOP_LINE_CONTENT: .byte 16         ; Updated by timer4 interrupt, used by LCD update loop
CURRENT_CHARSET_INDEX: .byte 16    ; Updated by timer4 interrupt, used by LCD update loop
CURRENT_CHAR_INDEX: .byte 1        ; Updated by timer4 interrupt, used by LCD update loop


; =============================================
; ======= END OF "DO NOT TOUCH" SECTION =======
